...
headers use `scope="colgroup"` and a correct `colspan`. `aria-rowcount` reports the full row
count even when only a page is rendered.
...
fire the row's click handler.

**Column resizing.** The grip is a `role="separator"` with an accessible name. Focus it and
use ← / → to resize (Shift for larger steps), Enter or Backspace to reset. Keyboard resizing
is real, not a token gesture.
