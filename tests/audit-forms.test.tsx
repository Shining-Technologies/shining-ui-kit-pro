...
describe('date and time, second audit', () => {
  ...
  it('stores the time the clock face shows when a day is picked first', async () => {
    const onChange = vi.fn()
    const user = userEvent.setup()
    render(
      <Themed>
    ...
    expect(screen.getByRole('spinbutton', { name: 'Minute' })).toHaveTextContent('00')
    const day = someDay()
    await user.click(day)
    expect(onChange).toHaveBeenLastCalledWith(`${day.dataset.day}T09:00`)
  })

  it('takes a defaultTime for both the dial and the value', async () => {
    const onChange = vi.fn()
    const user = userEvent.setup()
    render(
      <Themed>
    ...
    await user.click(screen.getByRole('button', { name: 'Appointment' }))
    expect(screen.getByRole('spinbutton', { name: 'Hour' })).toHaveTextContent('02')
