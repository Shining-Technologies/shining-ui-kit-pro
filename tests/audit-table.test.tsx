import { datePredicate, toTimestamp } from '@shining-technologies/ui-kit-core'
import { escapeCsvField, tableToCsv } from '@shining-technologies/ui-kit-export-csv'
import {
  DataTable,
  createColumnHelper,
  type ColumnDef,
  ...
  it('honours features.selection.getRowId', () => {
    render(
      <DataTable
        data={users}
        columns={userColumns}
        features={{ selection: { enabled: true, getRowId: (row) => `user-${row.id}` } }}
      />,
    )
...
describe('date filters', () => {
  ...
  it('reads a calendar date as that day in local time', () => {
    expect(toTimestamp('2024-03-05')).toBe(new Date(2024, 2, 5).getTime())
  })
