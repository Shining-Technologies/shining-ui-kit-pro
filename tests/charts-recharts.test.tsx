...
describe('the palette', () => {
  it('spreads the ramp instead of walking it, so adjacent series stay apart', () => {
    ...
    expect(seriesColor(0)).toBe('var(--sui-chart-1)')
    expect(seriesColor(1)).toBe('var(--sui-chart-3)')
    expect(seriesColor(2)).toBe('var(--sui-chart-5)')
  })
...
describe('StatTile', () => {
  it('separates which way a number moved from whether that is welcome', () => {
    const { container, rerender } = render(
      <StatTile label="Revenue" value={128400} delta={{ value: 12.4, period: 'vs last month' }} />,
    )
    expect(screen.getByText('128.4k')).toBeInTheDocument()
    expect(container.querySelector('.sui-viz-stat__delta')).toHaveAttribute('data-tone', 'positive')

    // The same rise, on a measure where rising is bad.
    rerender(<StatTile label="Churn" value={4.1} delta={{ value: 12.4, upIsGood: false }} />)
    expect(container.querySelector('.sui-viz-stat__delta')).toHaveAttribute('data-tone', 'negative')
  })
})
