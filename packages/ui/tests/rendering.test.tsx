...
describe('basic rendering', () => {
  ...
  it('leaves the totals footer off while there are no rows to total', () => {
    type Row = (typeof users)[number]
    const columns = [{ accessorKey: 'score', header: 'Score', footer: () => 'Total' }] as const
    const { container, rerender } = render(
      <DataTable<Row> data={users} columns={[...columns]} />,
    )
