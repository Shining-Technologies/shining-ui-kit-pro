import { DataTable, type ColumnFiltersState } from '@shining-technologies/ui'
...
describe('the date filter', () => {
  ...
  it('shows the picked preset as selected, on the trigger too, and unpicks it', async () => {
    const user = userEvent.setup()
    render(<DataTable data={users} columns={userColumns} filterLayout="inline" />)

    await user.click(screen.getByRole('button', { name: 'Created filter' }))
    const preset = await screen.findByRole('button', { name: 'Today' })
    ...
    expect(screen.getByRole('button', { name: 'Created filter' })).toHaveTextContent('Today')

    // Picking the selected preset again clears the filter.
    await user.click(screen.getByRole('button', { name: 'Today' }))
    expect(screen.getByRole('button', { name: 'Today' })).toHaveAttribute('aria-pressed', 'false')
    expect(screen.getByRole('button', { name: 'Created filter' })).toHaveTextContent('Any')
  })
