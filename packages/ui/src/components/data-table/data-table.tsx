...
import { DataTableBody } from './parts/structure'
...
export function DataTable<TData>(props: DataTableProps<TData>) {
  ...
  // Virtualization needs a body that renders from `rows` itself — the one in
  // `@shining-technologies/ui/virtualized`. With the default body the flag
  // would hand it no rows at all, so it is ignored rather than obeyed.
  const virtualizationIgnored =
    instance.features.virtualization.enabled && components.Body === DataTableBody
  const features = virtualizationIgnored
    ? {
        ...instance.features,
        virtualization: { ...instance.features.virtualization, enabled: false },
      }
    : instance.features
  useEffect(() => {
    if (!virtualizationIgnored || !isDevelopment()) return
    console.warn(
      '[shining-ui] `features.virtualization.enabled` has no effect on <DataTable />. ' +
        'Use <VirtualizedDataTable /> from `@shining-technologies/ui/virtualized`, or pass its ' +
        '`VirtualizedBody` as `components.Body`.',
    )
  }, [virtualizationIgnored])
  ...
  const headingActions = renderSlot(props.headingActions, table)
  const hasHeading = Boolean(props.title || props.description || props.icon || headingActions)

  const rootStyle: CSSProperties | undefined = props.style
  ...
  return (
    <DataTableProvider value={context}>
      <Root
        ...
        rootProps={{
          id: tableId,
          className: cn('sui-root', props.className),
          style: rootStyle,
