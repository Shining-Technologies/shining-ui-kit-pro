...
export interface DataTableContextValue<TData> {
  ...
  /** BCP 47 locale every number and date the table formats uses. */
  locale: string
  /** IANA zone date filters decide days in; `undefined` is the runtime's own zone. */
  timeZone: string | undefined
  /** Shared `Intl.NumberFormat` for `locale`. */
...
export function useDataTable<TData = unknown>(): DataTableContextValue<TData> {
  ...
  if (!context) {
    throw new Error(
      '[shining-ui] useDataTable() must be called inside <DataTable /> or <DataTableProvider />.',
    )
  }
