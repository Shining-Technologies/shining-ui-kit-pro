...
export function VirtualizedDataTable<TData>(props: DataTableProps<TData>) {
  return (
    <DataTable
      ...
      features={{
        ...props.features,
        // Merged, not replaced: passing `pageSize` alone must not switch
        // pagination back on. Only an explicit `enabled: true` does.
        pagination: { enabled: false, ...props.features?.pagination },
