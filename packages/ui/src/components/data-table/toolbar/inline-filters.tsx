...
import { DATE_RANGE_PRESETS, fromIso } from '../../date-time/date-utils'
...
import { SelectOptionItems } from '../filters/option-items'
import { useColumnFilter } from '../filters/use-column-filter'
import { optionValue } from '../filters/values'
...
function InlineFilter<TData>({ column, config, label }: InlineFilterProps<TData>) {
  ...
  const { locale } = useDataTable<TData>()
  ...
  function control() {
    ...
    if (config.type === 'select') {
      ...
      return (
        <Select
          value={current}
          onValueChange={(next) => (next === ANY ? clear() : setValue(optionValue(next, config)))}
        >
          <SelectTrigger
            className="sui-inline-filter__trigger"
            data-active={isActive || undefined}
    ...
    if (config.type === 'date' || config.type === 'number') {
      ...
      // The preset the current range is, if it is one — so "Last 7 days" stays
      // "Last 7 days" on the trigger and in the panel, rather than turning into
      // two dates the moment it is picked.
      const preset =
        isDate && !single && isActive
          ? DATE_RANGE_PRESETS.find((entry) => {
              const [from, to] = entry.range()
              return toText(range[0]) === from && toText(range[1]) === to
            })
          : undefined
      const summary = single
        ? (describe(filter.value, isDate, locale) ?? 'Any')
        : (preset?.label ?? rangeSummary(range[0], range[1], isDate, locale))
          ...
          >
                ...
                {DATE_RANGE_PRESETS.map((entry) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        // Picking the selected preset again is how it is undone.
                        if (selected) {
                          clear()
                          return
                        }
            ...
            {single ? (
              <label className="sui-range-panel__field">
                <span className="sui-range-panel__label">{operatorLabel(config.type, operator)}</span>
                {isDate ? (
                  <DateField
                    label={label}
...
function describe(value: unknown, isDate: boolean, locale: string): string | undefined {
  const text = toText(value)
  if (!text) return undefined
  if (!isDate) return text
  const date = fromIso(text)
  return date ? new Intl.DateTimeFormat(locale, { dateStyle: 'medium' }).format(date) : text
}

function rangeSummary(from: unknown, to: unknown, isDate: boolean, locale: string): string {
  const start = describe(from, isDate, locale)
  const end = describe(to, isDate, locale)
 ...
 * looked up for the column's own type (`on` for a date, `>` for a number).
 */
function operatorLabel(type: FilterType, operator: FilterOperator): string {
  return getOperator(type, operator)?.label ?? 'Value'
}
