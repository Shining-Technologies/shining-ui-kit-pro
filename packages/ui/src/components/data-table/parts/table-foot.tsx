...
/**
 * Renders `<tfoot>` when any column declares a `footer` and there are rows to
 * total. Under an empty, error or first-load body a row of `$0` totals is noise.
 */
export function TableFoot<TData>() {
  ...
  if (!hasFooter || table.getRowModel().rows.length === 0) return null
