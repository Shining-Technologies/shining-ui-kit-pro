...
export function useScrollEdges(): ScrollEdgesResult {
  ...
  const measure = useCallback(() => {
    ...
    if (!node) return
    // The visible width, for content that must span the viewport of a table
    // wider than its frame — the empty and error panels — rather than the
    // whole scrollable width. Written straight to the style: no re-render.
    node.style.setProperty('--sui-container-width', `${node.clientWidth}px`)
