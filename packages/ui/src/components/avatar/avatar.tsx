...
export interface AvatarGroupProps extends HTMLAttributes<HTMLDivElement> {
  ...
  max?: number
  /**
   * Extra classes for the `+n` chip. Its size already follows the avatars in
   * the group, so this is not needed for sizing.
   */
  moreClassName?: string
}

/**
 * Overlapping avatars — `Avatar`s or `UserAvatar`s.
 *
 ...
 *
 * The `+n` chip takes the size of the avatars beside it: the stylesheet reads
 * their size classes, so a row of large `UserAvatar`s gets a large chip.
 */
    ...
    >
      ...
      {hidden > 0 ? (
        <span
          ...
          className={cn('sui-avatar sui-avatar-group__more', moreClassName)}
        >
