...
import { PanelLeftIcon, SortIcon } from '../icons/icons'
import { initialOf } from './sidebar-tree'
import { useHydrated } from '../../hooks/use-hydrated'
import { Button, type ButtonProps } from '../button/button'
import { DropdownMenu, DropdownMenuContent, DropdownMenuTrigger } from '../overlay/dropdown-menu'
import {
  DEFAULT_BREAKPOINT,
  LinkRendererContext,
  SidebarContext,
...
import { UserAvatar, type PresenceStatus } from '../avatar/user-avatar'
import { isDevelopment } from '../../lib/dev'
import type { SidebarLinkProps } from './sidebar-nav'

/*
 * The dashboard sidebar: sections with their own colour, destinations nested
 * as deep as the product's information architecture goes, an icon rail that
 * still reaches every page through flyouts, a filter for when there are sixty
 * destinations, and a drawer on a phone.
 *
...
export interface SidebarProps
  ...
  'aria-label'?: string
  /**
   * Content-Security-Policy nonce for the small `<style>` a custom
   * `mobileBreakpoint` renders into server HTML. Applies with or without a
   * `SidebarProvider` above.
   */
  nonce?: string
  /**
   * Render links with your router: the `SidebarBrand` and `SidebarUser` links
   * in this sidebar, and every `SidebarNav` inside it without a `renderLink`
   * of its own.
   */
...
export const Sidebar = forwardRef<HTMLElement, SidebarProps>(function Sidebar(props, ref) {
  ...
  // Under a provider the provider owns the state: say so rather than drop props silently.
  const ignored = outer ? PROVIDER_KEYS.filter((key) => props[key] !== undefined).join(', ') : ''
  useEffect(() => {
    if (!ignored || !isDevelopment()) return
    console.warn(
      `Sidebar: ${ignored} ignored because a SidebarProvider is above this Sidebar. ` +
        'Set them on the SidebarProvider instead.',
    )
  }, [ignored])
  ...
  return (
    <>
      ...
      <div
        ...
        {...props}
      >
        <LinkRendererContext.Provider value={renderLink ?? null}>
...
const Identity = forwardRef<HTMLElement, IdentityProps>(function Identity(
  ...
  if (href && renderLink) {
    // `SidebarLinkProps` has no ref, so a router link does not receive this component's.
    return (
      <>
        {renderLink({
