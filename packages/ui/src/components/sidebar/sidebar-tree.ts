...
/**
 * Every item in the tree, depth first, with the chain of items above it and
 * the section it is in (`undefined` for an item outside any section).
 */
export function walk(
  ...
  const descend = (
    ...
    for (const item of items) {
      visit(item, ancestors, section)
      if (item.children?.length) descend(item.children, [...ancestors, item], section)
    }
  }
  for (const entry of entries) {
    if (isSeparator(entry)) continue
    if (isSection(entry)) descend(entry.items, [], entry)
    else descend([entry], [], undefined)
  }
}
