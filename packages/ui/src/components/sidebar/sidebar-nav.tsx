...
import { ChevronDownIcon, CloseIcon, ExternalLinkIcon, SearchIcon } from '../icons/icons'
import {
  filterEntries,
  getSidebarTrail,
  initialOf,
...
export interface SidebarNavProps extends Omit<HTMLAttributes<HTMLElement>, 'onSelect'> {
  ...
  /**
   * What the search announces to screen readers while it matches something.
   * Defaults to `"1 result"` / `"N results"`.
   */
  resultsMessage?: (count: number) => string
  /**
   * Render links with your router — `({ href, ...props }) => <Link to={href} {...props} />`.
   * Receives every prop the default `<a>` would have had. Defaults to the
   * `renderLink` of the enclosing `Sidebar`.
   */
  renderLink?: (props: SidebarLinkProps) => ReactNode
...
export const SidebarNav = forwardRef<HTMLElement, SidebarNavProps>(function SidebarNav(
  {
    ...
    resultsMessage = defaultResultsMessage,
    renderLink,
    onSelect,
    onKeyDown,
    children,
  ...
  const sidebarRenderLink = useContext(LinkRendererContext)
  ...
  // Which items share a parent, for accordion mode. Top-level items are grouped
  // by their section, so opening a branch in one section leaves the others be.
  const siblingsOf = useMemo(() => {
    const byParent = new Map<string, string[]>()
    const parentOf = new Map<string, string>()
    walk(items, (item, ancestors, section) => {
      const parent = ancestors.at(-1)?.id ?? (section ? `\0section:${section.id}` : '\0root')
      parentOf.set(item.id, parent)
      byParent.set(parent, [...(byParent.get(parent) ?? []), item.id])
    })
    return (id: string) => {
      const parent = parentOf.get(id)
      // A hand-written item that is not in `items` has no known siblings.
      return parent === undefined ? [] : (byParent.get(parent) ?? [])
    }
  }, [items])

  // The section holding the current page: it opens along with the branches.
  const activeSectionId = useMemo(() => {
    let found: string | undefined
    walk(items, (item, _ancestors, section) => {
      if (item.id === activeId) found = section?.id
    })
    return found
  }, [items, activeId])
  ...
  const linkRenderer = renderLink ?? sidebarRenderLink ?? undefined
  const navContext = useMemo<NavContextValue>(
    ...
    [expanded, filtered, setExpanded, trail, linkRenderer],
  )
    ...
    if (isSeparator(entry)) {
      ...
      blocks.push(
        <SectionRevealContext.Provider
          key={entry.id}
          value={entry.id === activeSectionId ? (activeId ?? null) : null}
        >
          <SidebarSection
  ...
  return (
    ...
    >
      ...
      {searchable ? (
        // Mounted with the search box, so the region exists before its text
        // changes: a live region inserted together with its message is often
        // not announced at all.
        <div role="status" className="sui-visually-hidden" data-slot="sidebar-nav-status">
          {filtered ? (filtered.count === 0 ? emptyMessage : resultsMessage(filtered.count)) : null}
        </div>
      ) : null}
      ...
      {filtered && filtered.count === 0 ? (
        // The status region above announces it; this is the visible copy.
        <p className="sui-sidebar-nav__empty" aria-hidden="true">
          {emptyMessage}
        </p>
      ) : null}
    </nav>
...
const defaultResultsMessage = (count: number) => (count === 1 ? '1 result' : `${count} results`)

/**
 * Set by `SidebarNav` around a generated section: the id of the current page
 * when it is inside that section, else `null`. A folded section opens when it
 * changes to an id, the way a branch on the trail does.
 */
const SectionRevealContext = createContext<string | null>(null)
...
export const SidebarSection = forwardRef<HTMLDivElement, SidebarSectionProps>(
  function SidebarSection(
    ...
    // Open from the first render when it holds the current page, so server HTML shows it.
    const [openState, setOpen] = useState(!defaultCollapsed || reveal !== null)
    // A new current page inside opens it once; the user can fold it again.
    useEffect(() => {
