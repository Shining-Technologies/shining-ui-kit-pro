...
/** Decimal places a number is written with: `1.25` has 2, `1e-7` has 7. */
...
export const NumberInput = forwardRef<HTMLInputElement, NumberInputProps>(function NumberInput(
  ...
  function nudge(direction: 1 | -1) {
    ...
    // rather than showing the artefact — at the finer of the step's decimals
    // and the value's own, so 1.5 stepped by 1 is 2.5, not 3.
    const decimals =
      precision ?? Math.min(100, Math.max(decimalsOf(step), decimalsOf(base)))
