...
> {
  ...
  disabled?: boolean
  /**
   * Announce the text box as required (`aria-required`). Not enforced by the
   * browser — the tags travel in hidden inputs — so check it on submit. Taken
   * from a surrounding `Field` when unset.
   */
  required?: boolean
  'aria-label'?: string
}
...
export const TagsInput = forwardRef<HTMLDivElement, TagsInputProps>(function TagsInput(
  ...
  function onPaste(event: ClipboardEvent<HTMLInputElement>) {
    ...
    const incoming = text
      ...
      .filter(Boolean)
    const next = [...tags]
    let rejected: { tag: string; message: string } | null = null
    for (const tag of incoming) {
      if (max !== undefined && next.length >= max) break
      if (dedupe && next.includes(tag)) continue
      const message = validate?.(tag, next) ?? null
      if (message) {
    ...
    commit(next)
    // The first tag that failed goes back in the box with its message, as a
    // typed one would, so it can be corrected rather than silently lost.
    setDraft(rejected?.tag ?? '')
    setError(rejected?.message ?? null)
  }
    ...
    <div className="sui-tags-wrap">
      ...
      >
        ...
        <input
          ...
          // `aria-required` rather than `required`: the box is empty once a tag
          // is added, and a native `required` would block a form that has tags.
