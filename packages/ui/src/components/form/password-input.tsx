...
import type { PasswordRule } from './password-rules'
...
export interface PasswordInputProps extends Omit<InputHTMLAttributes<HTMLInputElement>, 'type'> {
  ...
  rules?: PasswordRule[]
  /**
   * Show the per-rule checklist under the meter. On by default; `false` keeps
   * the meter and the verdict only. Ignored unless `strength` is set.
   */
  showRules?: boolean
  /**
   * Show the button that reveals the password. On by default; set `false` for
   * a confirmation field, where it adds nothing.
   */
  revealable?: boolean
