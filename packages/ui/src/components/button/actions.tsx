...
export interface CopyButtonProps extends Omit<ButtonProps, 'onCopy' | 'children'> {
  ...
  children?: ReactNode
  /** Visible label while the confirmation is showing. Not used by the icon-only button. */
  copiedLabel?: ReactNode
  /**
   * Announced through a live region once the copy succeeds. The region sits
   * outside the button's name, which stays `label` throughout.
   */
  ...
  /**
   * The button's accessible name (`aria-label`), with or without children, so
   * it does not change while the confirmation shows. Defaults to `children`
   * when that is a string, otherwise `'Copy'`. When you pass children, include
   * their visible text in it.
   */
  label?: string
}
...
export const CopyButton = forwardRef<HTMLButtonElement, CopyButtonProps>(function CopyButton(
  ...
  // One name throughout: the visible text swaps to `copiedLabel` and back, and
  // a name that flipped with it would be announced as a different control.
  ...
  return (
    <>
      ...
      {/* Beside the button, not in it: a button's descendants are presentational,
          so a live region inside one may never be announced. */}
