...
export const tableVariants = cva('sui-plain-table', {
  variants: {
    /**
     * Rhythm only — never colour or behaviour (§27). Pads header and body cells
     * alike. The names are the kit's `Density`; `'default'` is a deprecated
     * alias of `'comfortable'`.
     */
    density: {
      compact: 'sui-plain-table--compact',
      comfortable: '',
      /** @deprecated Use `'comfortable'`. */
      default: '',
  ...
  defaultVariants: { density: 'comfortable', striped: false, bordered: false },
})
...
export interface TableProps
  ...
  /**
   * Anything else for the wrapper: `style`, a `ref`. To make a wide table's
   * scroll region keyboard-reachable, pass `role: 'region'`, an `aria-label`
   * and `tabIndex: 0` — the role is what makes the label valid on a `<div>`.
   */
...
export interface TableRowProps extends HTMLAttributes<HTMLTableRowElement> {
  /**
   * Highlights the row and states it with `aria-selected`. `false` sets
   * `aria-selected="false"` (the row is selectable, not selected); leave it
   * unset on rows that cannot be selected.
   */
  selected?: boolean
}
