...
> {
  ...
  /**
   * - `'auto'` (default): no legend for a single series, because the title
   *   already names what is plotted; from two series up, a legend whose entries
   *   hide and show their series (a plain list when the chart cannot hide one).
   * - `'always'`: a plain list, even for one series.
   * - `'interactive'`: toggle buttons, even for one series.
   * - `false`: no legend.
   */
  legend?: 'auto' | 'always' | 'interactive' | false
  /**
   * Locale for the full-precision numbers in the tooltip and the table view.
   * Defaults to `'en-US'` — never the runtime's, so server and client agree.
   */
  locale?: string
...
interface ChartFrameProps extends BaseChartProps {
   ...
   */
  tableColumns?: string[]
  /** Formats table cells. Defaults to `formatFull` in `locale`. */
  valueFormatter?: ValueFormatter
  /**
   * Per-column formatters, by value-column index, for tables whose columns are
   * different measures (a scatter's x and y). A missing entry uses
   * `valueFormatter`.
   */
  tableColumnFormatters?: (ValueFormatter | undefined)[]
  children: (context: {
    size: ChartSize
    width: number
    height: number
...
export function ChartFrame({
  ...
  const formatCell = (value: number, column: number) =>
    (tableColumnFormatters?.[column] ?? valueFormatter ?? ((v: number) => formatFull(v, locale)))(value)
  ...
  // `'auto'` is interactive whenever it shows at all (two series up) and the
  // chart can hide a series; `'always'` is the explicit plain list.
  const interactive = (legend === 'interactive' || legend === 'auto') && Boolean(onToggleSeries)
          ...
          */}
          <div id={tableId} className={cn('sui-viz__table', !tableOpen && 'sui-visually-hidden')}>
            <table>
              ...
              <tbody>
                {/* Keyed by position: labels repeat (two points for one client,
                    two days with one formatted label) and must not collide. */}
                {tableRows?.map((row, r) => (
                  <tr key={`${r}-${row.label}`}>
                    <th scope="row">{row.label}</th>
                    {row.values.map((value, i) => (
                      <td key={i}>{value === null ? '—' : formatCell(value, i)}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
