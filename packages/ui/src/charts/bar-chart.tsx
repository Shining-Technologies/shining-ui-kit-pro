...
export interface BarChartProps extends BaseChartProps {
  ...
  sort?: 'asc' | 'desc'
  /**
   * Formats values. Given, it is used for axis ticks, value labels, the tooltip
   * and the table view (percent stacks keep percentages on the axis and in the
   * tooltip). Omitted, ticks and labels are compact and the tooltip and table
   * show full precision in `locale`.
   */
...
export function BarChart({
  ...
  const axisFormatter = percent ? (v: number) => `${Math.round(v)}%` : compactFormatter
            ...
            >
              ...
              <Tooltip
                ...
                content={
                  <ChartTooltipContent
                    labelFormatter={labelFormatter}
                    valueFormatter={percent ? (v) => `${v.toFixed(1)}` : fullFormatter}
                    unit={percent ? '%' : unit}
                  />
                }
              />
                ...
                >
                  {labelsFit ? (
                    <LabelList
                      ...
                      formatter={(value: number) => compactFormatter(value)}
                    />
                  ) : null}
                </Bar>
              ))}
