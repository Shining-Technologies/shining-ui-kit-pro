 ...
 *
 * The remaining shortfall is in the ramp, not the ordering. Until it is
 * regenerated, identity never rests on hue alone
 ...
 * stroke, so it has to match whatever sits behind the chart. Set
 * `--sui-chart-surface` on the chart or on any ancestor to change it; unset, it
 * falls back to `--card`. The fallback lives here, in the `var()`, rather than
 * in a stylesheet declaration on `.sui-viz` — a declaration on the chart would
 * shadow an ancestor's value, and a standalone `Sparkline` has no `.sui-viz`.
 */
export const CHART_SURFACE = 'var(--sui-chart-surface, var(--card))'

/** Gridlines and axis rules: one step off the surface, hairline, never dashed. */
export const AXIS_LINE_COLOR = 'var(--border)'
...
/**
 * The locale charts format numbers in unless given one. Fixed rather than the
 * runtime's default, which differs between a server and a browser and would
 * make server-rendered table text disagree with the first client render. The
 * same default as `DataTable`.
 */
export const DEFAULT_CHART_LOCALE = 'en-US'

/** Full precision with thousands separators, for tooltips and the table view. */
export function formatFull(value: number, locale: string = DEFAULT_CHART_LOCALE): string {
