...
export interface ScatterChartProps extends BaseChartProps {
  ...
  sizeKey?: string
  /** Names each point: the tooltip heading and the table view's row header. */
  labelKey?: string
  xLabel?: string
  yLabel?: string
  showGrid?: boolean
  /**
   * Formats x values. Given, it is used for ticks, the tooltip and the table
   * view. Omitted, ticks are compact and the tooltip and table show full
   * precision in `locale`.
   */
  xFormatter?: ValueFormatter
  /** Formats y values, as `xFormatter` does for x. */
...
export function ScatterChart({
  ...
  return (
    <ChartFrame
      ...
      tableColumnFormatters={[xFormatter, yFormatter]}
      locale={locale}
    >
      {({ size, label }) => {
            ...
            >
              ...
              <XAxis
                {...valueAxisProps({
                  size,
                  values: xValues,
                  formatter: xFormatter ?? formatCompact,
                  horizontal: true,
                })}
                dataKey={xKey}
                name={xLabel ?? xKey}
              ...
              />
              <YAxis
                {...valueAxisProps({ size, values: yValues, formatter: yFormatter ?? formatCompact })}
                dataKey={yKey}
                name={yLabel ?? yKey}
                label={
              ...
              <Tooltip
                ...
                content={
                  <ChartTooltipContent
                    ...
                    labelKey={labelKey}
                    locale={locale}
                    valueFormatters={{ [xKey]: xFormatter, [yKey]: yFormatter }}
                  />
                }
              />
