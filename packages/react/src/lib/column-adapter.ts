...
function resolveId<TData>(def: ColumnDef<TData>): string {
  ...
  if (key) return key
  throw new Error(
    '[shining-ui-kit] A column needs an `id` when it has no `accessorKey`. ' +
...
function wrapCell<TData>(cell: AnyCellRenderer<TData> | undefined) {
  return (context: EngineCellContext<TData, unknown>) => {
    const value = context.getValue()
    if (!cell) return defaultCellContent(value)
...
function wrapSortingFn<TData>(def: ColumnDef<TData>): SortingFn<TData> | undefined {
  const option = (def as { sortingFn?: unknown }).sortingFn
  if (option === undefined) return undefined
  const compare = resolveComparator(option as never)
  return (rowA, rowB, columnId) => compare(rowA.getValue(columnId), rowB.getValue(columnId))
}
...
function adaptOne<TData>(
  ...
  if (isGroup(def)) {
    ...
    return {
      ...
      meta: def.meta,
      columns: def.columns.map((child) => adaptOne(child, out)),
  ...
  const column: Record<string, unknown> = {
    ...
    header: wrapHeader<TData>(def.header),
    footer: wrapHeader<TData>(def.footer),
    cell: wrapCell<TData>(def.cell as AnyCellRenderer<TData> | undefined),
    meta,
    ...
    sortingFn: wrapSortingFn(def),
    filterFn: def.filter ? createColumnFilterFn(def.filter) : undefined,
  }
